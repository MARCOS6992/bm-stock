  ...
  async function loadData() {
    const { data, error } = await supabase
      ...
      .select(`id, ref, designation, numero_serie, fournisseur,
        bon_pose:bons_pose(numero, client, code_cee, technicion, created_at,
  ...
  const filteredRows = search
    ? rows.filter((r) => {
        ...
        return r.client.toLowerCase().includes(q) || r.ref.toLowerCase().includes(q) ||
          ...
          r.technicion.toLowerCase().includes(q) || r.sous_traitant.toLowerCase().includes(q) ||
          r.numero_bon_pose.toLowerCase().includes(q) || (r.fournisseur || '').toLowerCase().includes(q) ||
              ...
              {filteredRows.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">
                    {row.date ? format(new Date(row.date), 'dd/MM/yyyy', { locale: fr }) : <span className="text-gray-300">—</span>}
                  </td>
                </tr>
              ))}
            </tbody>
