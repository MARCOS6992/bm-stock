...
export default function HistoriquePage() {
  ...
  async function loadData() {
    const { data, error } = await supabase
      .from('lignes_pose')
      .select(`id, ref, designation, numero_serie, fournisseur,
        bon_pose:bons_pose(numero, client, code_cee, date_pose, created_at,
    ...
    if (data) {
      setRows(data.map((item: any) => ({
        ...
        date_pose: item.bon_pose?.date_pose || item.bon_pose?.created_at || null,
      })))
    }
    setLoading(false)
  }
  ...
  const filteredRows = search
    ? rows.filter((r) => {
        ...
        return r.client.toLowerCase().includes(q) || r.ref.toLowerCase().includes(q) ||
          r.designation.toLowerCase().includes(q) || (r.numero_serie || '').toLowerCase().includes(q) ||
          r.sous_traitant.toLowerCase().includes(q) || r.numero_bon_pose.toLowerCase().includes(q) ||
          (r.fournisseur || '').toLowerCase().includes(q) || (r.code_cee || '').toLowerCase().includes(q)
      })
    : rows
  ...
  return (
    <div>
      <PageHeader title="Historique des poses"
        subtitle={`${rows.length} article${rows.length !== 1 ? 's' : ''} posé${rows.length !== 1 ? 's' : ''}`} />
      <div className="mb-4">
        <input type="search" value={search} onChange={(e) => setSearch(e.target.value)}
          placeholder="Rechercher par client, produit, numéro de série..."
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredRows.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    {row.sous_traitant && <span className="inline-block px-2 py-0.5 rounded text-white text-xs font-medium" style={{ backgroundColor: row.couleur_st }}>{row.sous_traitant}</span>}
                  </td>
