...
export default function NewReceptionPage() {
  ...
  function addLigne() {
    ...
    const series = produit.necessite_serie ? Array(qte).fill('') : []
    setLignes((prev) => [
      ...prev,
      { produit, qte, series },
    ])
  ...
  function updateQte(ligneIdx: number, newQte: number) {
    setLignes((prev) =>
      prev.map((l, i) => {
        if (i !== ligneIdx) return l
        const series = l.produit.necessite_serie
          ? Array(newQte).fill('').map((_, idx) => l.series[idx] || '')
          : []
  ...
  async function submit() {
    ...
    try {
      ...
      // Generate numero
      const { data: lastBr } = await supabase
        .from('bons_reception')
        .select('numero')
        .order('numero', { ascending: false })
        .limit(1)
      let seq = 1
      if (lastBr && lastBr.length > 0) {
        const match = lastBr[0].numero.match(/(\d+)$/)
        if (match) seq = parseInt(match[1]) + 1
      }
      const numero = `BR-${new Date().getFullYear()}-${String(seq).padStart(4, '0')}`

      // Insert bon reception
  ...
  const seriesComplete = lignes.every((l) =>
    !l.produit.necessite_serie || l.series.every((s) => s.trim() !== '')
  )
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="ml-2 text-sm text-gray-500">
          {step === 1 ? 'Informations BL' : step === 2 ? 'Articles reçus' : 'Signature'}
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={distributeur} onChange={(e) => setDistributeur(e.target.value)}
            ...
            <div>
              ...
              <input value={blFournisseur} onChange={(e) => setBlFournisseur(e.target.value)}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={receptionnePar} onChange={(e) => setReceptionnePar(e.target.value)}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <input type="number" value={qteInput} onChange={(e) => setQteInput(e.target.value)}
              ...
              {lignes.map((ligne, ligneIdx) => (
                <div key={ligneIdx} className="bg-white rounded-xl border border-gray-100 p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      <input type="number" value={ligne.qte}
                        onChange={(e) => updateQte(ligneIdx, parseInt(e.target.value) || 1)}
                        min="1"
                        className="w-16 border border-gray-200 rounded px-2 py-1 text-sm text-center" />
                      <button onClick={() => removeLigne(ligneIdx)}
                    ...
                    <div className="space-y-1 mt-2">
                      ...
                      {ligne.series.map((s, si) => (
                        <input key={si} value={s}
                          onChange={(e) => updateSerie(ligneIdx, si, e.target.value)}
                          placeholder={`N° série ${si + 1}`}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 p-6">
            ...
            <div className="text-sm text-gray-600 space-y-1 mb-4">
              ...
              {lignes.map((l, i) => (
                <p key={i} className="pl-2 text-xs">• {l.produit.designation} × {l.qte}</p>
              ))}
            </div>
            <div>
