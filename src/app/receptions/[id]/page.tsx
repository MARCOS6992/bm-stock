...
export default function ReceptionDetailPage({ params }: { params: { id: string } }) {
  ...
  async function loadData() {
    const { data: b } = await supabase.from('bons_reception').select('*, sous_traitant:sous_traitants(*)').eq('id', params.id).single()
    if (b) setBon(b)
    const { data: ls } = await supabase.from('lignes_reception').select('*, produit:produits(*)').eq('bon_reception_id', params.id)
    if (ls) setLignes(ls)
    setLoading(false)
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          ...
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between"><dt className="text-gray-500">Distributeur</dt><dd className="font-medium">{bon.distributeur}</dd></div>
            <div className="flex justify-between"><dt className="text-gray-500">N° BL</dt><dd className="font-medium font-mono">{bon.bl_fournisseur}</dd></div>
            <div className="flex justify-between">
              <dt className="text-gray-500">Date réception</dt>
              <dd className="font-medium">{bon.date_reception ? format(new Date(bon.date_reception), 'dd/MM/yyyy', { locale: fr }) : '—'}</dd>
            </div>
            <div className="flex justify-between"><dt className="text-gray-500">Réceptionné par</dt><dd className="font-medium">{bon.receptionne_par}</dd></div>
            ...
            {bon.notes && <div><dt className="text-gray-500 mb-1">Notes</dt><dd className="text-gray-800">{bon.notes}</dd></div>}
          </dl>
        </div>
        {bon.signature_url && (
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              ...
              <div key={ligne.id} className="px-4 py-3">
                ...
                {ligne.series && ligne.series.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {ligne.series.map((s, i) => <span key={i} className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-mono">{s}</span>)}
                  </div>
                )}
              </div>
            )
