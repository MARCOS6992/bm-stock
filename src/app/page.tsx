  ...
  async function loadData() {
    try {
      ...
      if (sousTraitants && stockItems) {
        const summaries: StockSummary[] = sousTraitants.map((st) => ({
          sousTraitant: st,
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold" style={{ color: sousTraitant.couleur }}>
                    {count}
                  </p>
      ...
      {lowStockAlerts.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-gray-700 mb-3 flex items-center gap-2">
            <span className="text-yellow-500">⚠️</span> Alertes stock bas
          </h2>
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 space-y-2">
            {lowStockAlerts.map(({ produit, total }) => (
              <div key={produit.id} className="flex items-center justify-between text-sm">
                <span className="font-medium text-yellow-800">{produit.nom}</span>
                <span className="text-yellow-600">
                  {total} / seuil {produit.seuil_min}
                </span>
              </div>
            ))}
          </div>
                ...
                <div key={reception.id} className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-500">
                      {format(new Date(reception.created_at), 'dd/MM/yyyy', { locale: fr })}
                    </span>
                  </div>
                  <p className="text-sm text-gray-500 mt-1">
                    {reception.fournisseur} →{' '}
                    {(reception.sous_traitant as SousTraitant)?.nom}
                  </p>
                </div>
              ))
            )}
              ...
              recentPoses.map((pose) => (
                <div key={pose.id} className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-500">
                      {format(new Date(pose.created_at), 'dd/MM/yyyy', { locale: fr })}
                    </span>
                  </div>
                  <p className="text-sm text-gray-500 mt-1">
                    {pose.client} — {(pose.sous_traitant as SousTraitant)?.nom}
                  </p>
                </div>
              ))
            )}
