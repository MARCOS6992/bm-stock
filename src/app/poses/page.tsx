...
export default function PosesPage() {
  ...
  async function loadData() {
    const { data } = await supabase.from('bons_pose').select('*, sous_traitant:sous_traitants(*)').order('created_at', { ascending: false })
  ...
  return (
    <div>
      <PageHeader title="Bons de pose" action={
        <Link href="/poses/new" className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-700 transition-colors">+ Nouveau</Link>
      } />
      {poses.length === 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y divide-gray-100">
          {poses.map((p) => (
            <Link key={p.id} href={`/poses/${p.id}`} className="flex items-center justify-between px-4 py-4 hover:bg-gray-50 transition-colors">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-gray-900">{p.numero}</span>
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium"
                    style={{ backgroundColor: (p.sous_traitant as SousTraitant)?.couleur + '20', color: (p.sous_traitant as SousTraitant)?.couleur }}>
                    {(p.sous_traitant as SousTraitant)?.nom}
                  </span>
                </div>
